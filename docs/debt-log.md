...
### D20: `audit_logs` is mutable + seeded with a plaintext PAN — MITIGATED

| Field | Value |
|---|---|
| **ID** | D20 |
| **Finding** | *(As found 2026-07-19, and no longer true of the schema — see Status.)* `audit_logs` was an ordinary table — `UPDATE`/`DELETE` allowed, shipped a `deleted_at` soft-delete column, no append-only trigger (contrast `decision_events`, which has one). App code could silently tombstone/alter audit rows. Separately, the seed wrote a plaintext PAN into `audit_logs.detail`. |
| **Location** | `db/init/001_schema.sql` (`audit_logs` table + trigger); `db/migrations/0022_audit_logs_append_only.sql`; `db/init/002_seed.sql` (`audit_logs` seed row). |
| **Trigger** | Any `db.query("DELETE FROM audit_logs WHERE …")` / `UPDATE audit_logs …` used to succeed unresisted (proven reachable by the same helper used for `UPDATE applications`). |
| **Risk** | **High.** README claims "SOX-controlled with full audit" — the audit trail was forgeable, and it already contained raw cardholder data (compounds D13). |
| **Attribution** | **Pre-existing** (baseline `e8bb2fa` schema + seed). We added the append-only `decision_events` alongside (ADR 0009) but did not convert `audit_logs` at the time. |
| **Mitigation Path** | Add the `decision_events`-style `BEFORE UPDATE OR DELETE OR TRUNCATE` trigger to `audit_logs`; scrub the seeded PAN. |
| **Status** | **Mitigated.** `trg_audit_logs_append_only` / `trg_audit_logs_no_truncate` mirror the `decision_events` triggers byte-for-byte in both `db/init/001_schema.sql` and migration 0022, so a fresh install and a hand-applied volume agree. `origination-service` — the table's only writer (`app/routers/applications.py`) — gains a readiness rung (`app/config.py`) that resolves the trigger via `to_regclass('audit_logs')` (search-path-safe) and reports `schema_not_ready:trg_audit_logs_append_only` on a volume that predates it, so an unmigrated deployment is visible at `/health` rather than silently unguarded; regression coverage in `services/origination-service/tests/test_db_readiness.py`. Held by the blocking `db-readiness-gate` (origination-service is already in its matrix). The seeded PAN is masked the way `PiiRedactor` renders one (`••••••••••••1111 (PAN)`) rather than left plaintext. `deleted_at` is left on the table, now inert (any UPDATE it would need is blocked the same as every other UPDATE) — column shape was out of scope for this change. **Not covered:** pre-fix rows written before the trigger existed, and any backup/WAL predating it — same residual class as D13a's rewrite. |

### D21: Postgres and Redis host-published in the base compose — BOTH CLOSED (D21a, D21b)

*(Split on the fix. The original entry was one sentence — "Anyone on host/LAN **with DB creds**
bypasses all app-layer authz" — rated **Medium**. That sentence describes the Postgres half and
the Redis half silently inherited its precondition. Redis here has no credential to require, so
the rating was wrong for a month on the more severe of the two. Recorded rather than quietly
corrected, because the mis-rating is the reusable lesson: a two-datastore entry needs two risk
rows, or the safer half sets the severity for both.)*

| Field | Value |
|---|---|
| **ID** | D21a (Postgres) |
| **Finding** | Backend app ports (8001–8006) are correctly `expose`-only, but `postgres:5432` was published to the host in the base `docker-compose.yml`. Anyone on host/LAN with DB credentials reads and writes the schema directly, underneath the gateway trust boundary and underneath every app-layer authz check. There is one shared database and one schema (ADR 0002), so a direct connection is not a slice of the platform, it is all of it — including `applicants.ssn` and `payments.pan`, both plaintext (**D33**, **D13b**). |
...
| **Status** | **Mitigated.** The base compose publishes no host port for postgres. Held by `scripts/tests/test_compose_datastore_ports.py` in the blocking `compose-hardening-gate`, which grades the RESOLVED config (anchors, aliases and `<<` merge keys applied, as Compose applies them) and keys on the presence of a `ports` key rather than on its value, so every spelling is covered and an inherited port cannot pass a source-text scan. It refuses `network_mode` on a datastore under the same presence rule: `host` reaches 5432 through the host's own network namespace with no `ports` key at all, and `service:`/`container:` hide which namespace publishes it — a ports-only rule passes both. It also refuses a datastore joining a network the file cannot prove is a private bridge (`driver: macvlan` is a LAN address of its own; `external: true` is created outside the file), while accepting the default driver so a private `backend` network stays available. And it grades every file name Compose reads — `compose.yaml` takes precedence over `docker-compose.yml`, so a `docker-compose*.yml` glob left the more authoritative file ungraded. See the shared residual on D21b. |

| Field | Value |
|---|---|
| **ID** | D21b (Redis) |
| **Finding** | `redis:6379` was published to the host in the base `docker-compose.yml`, and **this half needs no credential at all**: the base file runs `redis:7-alpine` with no `requirepass` and `REDIS_URL` (`.env.example`) carries no auth. What lives there is not derived cache. The gateway stores two bearer credentials in it (`services/gateway/app/auth.py`): `session:{token}` → the resolved session including `role`, so `KEYS 'session:*'` yields a live `admin`/`underwriter` session; and `resume:{sid}` → `{app_id, token}` where `token` is the **raw** continuation token, the bearer capability for the money-moving routes. |
| **Location** | `docker-compose.yml` redis service. Writers: `services/gateway/app/auth.py` `create_session` (`session:`) and `create_resume_session` (`resume:`). |
| **Risk** | **High**, and higher than D21a. No credential, no cracking, no crypto in the way — a reachable 6379 is a direct read. It also **defeats a control that was built on purpose**: `origination-service/app/authz.py::hash_token` stores only a keyed digest of the continuation token specifically so that "a DB read / backup / logged row then yields only a non-replayable digest", and the raw token sat one datastore over behind an open port. A hashed credential in Postgres and a raw copy in an unauthenticated Redis is the weaker of the two, not the average. Every other item on this page needs database access first; this one hands over a session that grants it. |
| **Attribution** | **Pre-existing** (baseline compose), but the exposure grew after the baseline: the `resume:` key space was added later (ADR 0010 Phase B) and put a money-route capability into a datastore whose exposure nobody re-read. |
| **Mitigation Path** | Drop the `ports:`, keep `expose`; use `docker compose exec redis redis-cli` locally. Set `requirepass` and give `REDIS_URL` credentials as a separate change — see Residuals. |
| **Status** | **Mitigated.** The base compose publishes no host port for redis. Held by the same blocking test as D21a. |
| **Residuals** | Two, and both are the reason this is Mitigated and not Fixed. (1) **Redis still has no authentication.** Removing the published port removes host/LAN reachability; it does not add a credential. Anything that reaches the compose network still reads both key spaces unauthenticated — a fourth service, a sidecar, a misconfigured network, or an attacker with any container foothold. Setting `requirepass` plus a credentialed `REDIS_URL` is its own change. (2) **Nothing is encrypted in transit** between any service and either datastore (**D35**), so the credentials above cross the network in the clear regardless of who is listening. |
...
| **High** | D20: `audit_logs` mutable + seeded plaintext PAN | **Mitigated** | Teeth 2026-07-19. No append-only trigger (had `deleted_at`), rows UPDATE/DELETE-able; seed wrote raw PAN into `detail`. Forgeable "audit" contradicted README SOX claim. Closed by `trg_audit_logs_append_only`/`trg_audit_logs_no_truncate` (mirrors `decision_events`, migration 0022), a readiness rung in `origination-service` held by the blocking `db-readiness-gate`, and a masked seed value. |
| **High** | D21: Postgres/Redis host-published in base compose | **Mitigated 2026-08-29; split into D21a/D21b** | Teeth 2026-07-19, re-rated on the fix. Neither datastore publishes a host port; `expose` documents the container port instead, held by `scripts/tests/test_compose_datastore_ports.py` in the blocking `compose-hardening-gate`. **The rating was wrong at Medium for a month**: the entry's one sentence said "anyone on host/LAN *with DB creds*", which describes D21a — D21b needed no credential, because redis runs with no `requirepass` and held both a live officer session (`session:*`) and the RAW continuation token that authorizes the money-moving routes (`resume:*`), the same value origination hashes before storing it in Postgres. Not Fixed: redis still has no authentication at all (removing the port removed reachability, not the missing credential), and nothing is encrypted in transit (**D35**). |
...
| **High** | D27: Passwords are unsalted SHA-256 | **Mitigated** | Sweep, 2026-08-22. `gateway/app/auth.py` was a bare single-round `sha256(password)` with no salt and no work factor. One precomputed table covered every user, and identical passwords produced identical hashes. Closed by salted PBKDF2-HMAC-SHA256 (stdlib, no new dependency), 600k iterations, with rehash-on-successful-login for existing rows. Held by the blocking `gateway-trust-boundary-gate`. |
| **Medium** | D28: Equal nested timeouts, so the outer one can never fire first | **Partially fixed** (ours, partly) | Sweep, 2026-08-22. `origination-service/app/clients.py:28` sets `_TIMEOUT = 30.0` on every downstream call; `decision-service/app/decision.py` gave the bureau pull the same 30, now `_BUREAU_TIMEOUT = 20.0`, strictly below origination's budget, and origination's `run_decision` now catches the bare-timeout case instead of letting it fall through to a generic 500 — commit `6457328`, not yet merged. No retry anywhere in origination — which is right for the billable, inquiry-generating bureau pull and unexamined for everything else; that residual, plus the connect/read split and deadline propagation, are still open. Compounds ADR 0009 §6's deferred load behaviour. Found stale `CLAUDE.md` text alongside it ("no timeout/retry contract"), corrected. |
...
| **Medium** | D29: The set of controls that cannot regress is a hand-maintained allowlist | Open (ours) | Sweep, 2026-08-22. `.github/workflows/ci.yml:30,33` runs the whole matrix under `continue-on-error` + `\|\| true`, so protection means hand-copying a control into its own blocking job and the default for a new one is unprotected. This is the mechanism the 4.5pp APR defect came through, and `CLAUDE.md` names the live instance: `adjust_balance` (D32) keeps the unlocked read-modify-write shape — `apply_payment` and the red D3 lost-update test moved into the blocking `atomic-apply-gate` on 2026-08-24 (PR #77), and `waive_fee`'s own D32 half followed it into that same gate (`cd71243`, not yet merged). Not an argument for flipping the flag — the tolerated failures are deliberate D2 documentation. Fix: enumerate the known-red tests and let everything outside that list block. |
| **Medium** | D30: A crash between claiming an idempotency key and finishing the capture strands the row, and its key is never released | Open (ours, introduced by the D19 fix) | 2026-08-22. The D19 claim writes `status='processing'` before the capture completes. A crash in between leaves that row non-terminal forever, and **only a terminal intent releases its key** — so every later retry under that key answers `409` rather than replaying or charging, and the customer cannot pay. Smaller than the double charge it replaces, but genuinely new. Fix: the reaper `docs/specs/payments-week5.md` D2 specifies — resolve rows older than `PAYMENT_PROCESSING_TIMEOUT_MINUTES` by querying the processor for that row's `processor_idempotency_key` (stamped at insert precisely so it survives this crash), then retire expired keys in the same pass, stuck rows first. Deliberately out of scope for the D19 PR to keep it reviewable; carded rather than shipped silently. |
| **High** | D33: `applicants.ssn` stored in plaintext | Open (pre-existing) | Encryption sweep, 2026-08-29. `db/init/001_schema.sql` declares `ssn TEXT, -- plaintext`; one shared schema, so all 7 services read it with one credential. **No entry on this page ever covered the column** — every prior SSN row is about the logs (D5), the redactor (D22) or the decision inputs (D26), so the register read as though the SSN were handled. Largest breach blast radius here (every applicant has one; only payers have a PAN). `decision-service` already refuses to persist it unkeyed and stores a peppered HMAC in `decision_events` — correct, and not a substitute for the source column. **GLBA Safeguards Rule §314.4(c)(3) requires encryption of customer information at rest**, with a written Qualified-Individual exception as the only out — so this is a named control requirement, not only a breach-risk call, and "leave it" is not an option the client can select. Decide retention duration first: purging to last-4 beats encrypting, and is the only path needing no key management. Nothing blocking holds this. |
| **Low → Medium on D13b** | D34: payment request fingerprint is an unpeppered SHA-256 of the PAN | Open (ours, from the D19 fix) | Encryption sweep, 2026-08-29. Both charge handlers derive `instrument = sha256(pan)` with no pepper, while `decision-service` peppers its SSN digest against the identical brute-force argument (BIN + Luhn leaves ~10⁹ candidates). Harmless today — the digest sits in the same row as the plaintext `pan` — and becomes the card-derived value left behind the moment D13b drops that column. Fix inside the D13b change, where the fingerprint moves to `card_token` and the problem disappears by construction. |
| **Medium** | D35: nothing is encrypted in transit | Open (pre-existing) | Encryption sweep, 2026-08-29. All four hops are plaintext: browser→gateway and gateway→services are `http://`, `DATABASE_URL` carries no `sslmode` so psycopg2's default `prefer` negotiates down against an image shipping `ssl` off, and Redis is `redis://` not `rediss://`. **This page carried zero TLS entries before now**, so the gap read as covered. Medium only because every hop is one Docker bridge network on one host; High the moment any hop crosses a machine. GLBA §314.4(c)(3) qualifies the in-transit half as "over **external** networks", so a single-host bridge network is most likely outside it and the deferral holds — **until the first hop crosses a machine**, which is the scheduling trigger. Deployment posture, not a local code fix. |
...
| **Not the shared volume itself** | The shared volume is deliberate and is not the defect. `docker-compose.yml:1` pins the Compose project name, so every `make up` targets one stack from any directory. That is the correct behaviour here: the base compose publishes host ports 8000 and 3000 (5432 and 6379 came off in D21), so only one stack can run on a machine regardless, and without the pin each checkout would start a separate project that fails to bind those ports and leaves an orphan data volume behind. The pin dates to the initial scaffold (`e8bb2fa`), not to a later patch. |
...
### D27: Passwords are unsalted SHA-256 — MITIGATED

| Field | Value |
|---|---|
| **ID** | D27 |
| **Finding** | *(As found 2026-08-22, and no longer true of `main` — see Status.)* `hash_password` was a bare `hashlib.sha256(password.encode("utf-8")).hexdigest()` — no salt, no key-derivation function, no work factor. Login compared the stored hex to a freshly computed one with `!=`. The schema column comment stated the scheme outright. |
| **Location** | `services/gateway/app/auth.py` (`hash_password`, `_verify_password`, `authenticate`); `db/init/001_schema.sql` (`password_hash TEXT NOT NULL`, comment updated). |
| **Risk** | **High** at the time. Unsalted single-round SHA-256 over human-chosen passwords is recoverable at GPU speed from precomputed tables — cracking is not the hard part, obtaining the table is. Two properties made it worse than slow hashing: one rainbow table covered every user at once, and identical passwords produced byte-identical hashes, so password reuse across accounts was visible without cracking anything. The affected roles include `admin` and `underwriter`, which are the money-action roles. |
| **Reachability** | Any read of `users` is sufficient. All seven services connect to the one shared Postgres with the same credentials (ADR 0002), so a single service compromise reaches the table; the base compose no longer publishes 5432 to the host (D21a), which raises the bar but does not remove the reachability a compromised service already has; and D1's credentials remain unrotated. No SQL-injection path is needed for the exposure to matter. |
| **Attribution** | **Pre-existing** (baseline Halcyon auth). Deliberately kept and *documented in the module docstring* — but never entered in this log until 2026-08-22, so nothing had scheduled it. |
| **Mitigation Path** | Argon2id/bcrypt/PBKDF2 with a per-user salt and a tuned work factor, plus **rehash-on-successful-login** so existing rows drain without a forced org-wide reset. Use `hmac.compare_digest` for the comparison while the legacy branch survives. |
| **Status** | **Mitigated.** Salted PBKDF2-HMAC-SHA256 (stdlib `hashlib.pbkdf2_hmac`, 600,000 iterations — OWASP's 2023 minimum, no new dependency per the YAGNI rule), format `pbkdf2_sha256$<iterations>$<salt_hex>$<hash_hex>`. `_verify_password` dispatches on the stored value's prefix: a new-format hash verifies via PBKDF2 with `hmac.compare_digest`; a legacy bare-hex row still verifies against the old scheme so existing accounts keep authenticating. `authenticate()` rehashes a successful legacy match in place (`UPDATE users SET password_hash = …`), so every account drains to the new format on its next login with no forced reset — the seeded demo users (`db/init/002_seed.sql`, still shipping the legacy sha256 literal on purpose, so a fresh volume proves the migration path) included. Regression coverage in `services/gateway/tests/test_auth.py`: salting, rejection of the bare-sha256 shape, legacy-row acceptance and rejection, the rehash-on-login write, and the rehashed value re-verifying. Held by the blocking `gateway-trust-boundary-gate`. **Not covered:** identical passwords across accounts remain undetectable pre-migration (inherent to any per-row rehash-on-login scheme — there is no bulk migration pass); WAL/backups taken before a given row's rehash still hold its unsalted hash. |
...
|---|---|
| **ID** | D28 |
| **Finding** | Origination calls kyc, decision and disclosure through one httpx seam with a module-level `_TIMEOUT = 30.0` applied to every helper. Decision-service's own bureau call used `timeout=30` as well. The outer budget equalled the inner one rather than being strictly different, so on a slow bureau the inner call was what expired — origination could not distinguish "the bureau is slow" from "decision-service is dead", and no deadline was passed down or decremented across the hop. There is still no retry or backoff anywhere in origination. |
| **Location** | `services/origination-service/app/clients.py:28` (`_TIMEOUT = 30.0`); `services/decision-service/app/decision.py` (`_BUREAU_TIMEOUT = 20.0`, was `timeout=30` inline at the bureau `httpx.get` call). |
...
| **Mitigation Path** | Two directions close the equal-nesting collision; this fix takes the second because cutting origination off while a regulated, billable bureau pull is still legitimately in flight (the first direction) discards a completed inquiry the applicant is charged for anyway. (1) Give the outer call a budget strictly smaller than the inner one, so the caller attributes the stall itself. (2) **Shipped:** shorten the inner bureau-pull timeout to stay below the outer budget (`_BUREAU_TIMEOUT = 20.0` vs origination's `_TIMEOUT = 30.0`), so decision-service's own bounded refusal reaches origination before origination's timeout ever could. **Still open, either way:** split connect from read timeouts; state the retry policy per call class explicitly (no retry on the paid bureau pull, bounded retry on idempotent reads) rather than leaving it implicit; propagate an explicit deadline downstream instead of two independently-chosen constants. |
| **Status** | Partially fixed. The equal-nesting collision itself is closed: decision-service's bureau-pull timeout is now 20s, strictly below origination's 30s outer budget, and origination-service's `run_decision` route now catches the bare-timeout case (`httpx.HTTPError`, no response) it previously let fall through uncaught to a generic 500 — both proven by `make prove` (`services/decision-service/tests/test_bureau_timeout.py`, `services/origination-service/tests/test_decision_route.py::test_downstream_timeout_maps_to_503_not_an_unhandled_500`). Commit `6457328` on branch `fix/downstream-timeout-budget`, not yet merged. Residuals named above (connect/read split, explicit per-class retry policy, deadline propagation) are not started. |
...
| **Risk** | **Medium**, and it is the mechanism behind a shipped Reg Z defect rather than a hypothetical: the add-on-vs-actuarial APR error (4.5pp, roughly 36× the disclosure tolerance, on every loan) survived because the money test that would have caught it ran under `\|\| true`. `CLAUDE.md` names the current instance in its own words — `adjust_balance` (D32) is "still inside the suppression, and able to regress on a green build"; `apply_payment` and the red D3 lost-update test left it on 2026-08-24 for the blocking `atomic-apply-gate` (PR #77), and `waive_fee`'s D32 half has now joined them there (`cd71243`, not yet merged). |
...
### D32: `adjust_balance` keeps the read-modify-write shape D3 removed from `apply_payment`
...
|---|---|
| **ID** | D32 |
| **Finding** | D3 made `apply_payment` one statement, but the other two mutations of the same `balances` row were deliberately left alone. `adjust_balance` reads the current value, then writes an operator-supplied figure with no predicate on what it read — still true. `waive_fee` used to read `past_due`, subtract in Python, and write the result back; that half is now fixed (see Status). |
| **Location** | `services/servicing-service/app/balance.py:163-171` (`adjust_balance`, still read-modify-write). Reached via `POST /accounts/{id}/adjust-balance` (`app/main.py:228`), role-gated by `authz.require_money_role`. |
| **Trigger** | An operator `adjust_balance` call landing across a concurrent `apply_payment` — the operator's `UPDATE balances SET balance = %s` writes a figure computed before the payment committed, so an applied payment is silently reversed even though the apply itself was atomic. Two concurrent `adjust_balance` calls on the same loan also still race, last write wins. |
| **Risk** | **Medium**, below D3. There is no live reproduction, and the path is not on the borrower-facing payment flow: it is a servicer-initiated correction, staff-paced and low-volume, and `require_money_role` limits who can open the window. `adjust_balance` is destructive by design (it overwrites the figure and records the prior value nowhere), so with no ledger (D2) and no `audit_logs` write anywhere in `servicing-service` nothing can attribute a lost update afterwards. |
| **Attribution** | **Pre-existing** (baseline servicing), same as D3. Scoped out of ADR 0020 and the D3 fix on purpose rather than missed: the ADR fixed the path where money was measurably lost, and widening it to the correction paths would have grown a breaking cross-service change with no defect behind it. |
| **Mitigation Path** | `adjust_balance` sets an absolute figure, so D3/D32's atomic-decrement fix does not apply as-is — it needs the balance it was quoted from as a predicate (a compare-and-set on the value the operator saw, refusing when the row moved underneath), which is a UI-visible behaviour change and needs the client's answer on what an operator sees when it refuses. The deeper question is whether an absolute set should exist at all once `payment_applications` establishes the ledger seam (D2); take it with the append-only ledger (ADR 0014, Proposed, not built), which is what would make it reversible. |
| **Status** | Open for `adjust_balance`. `waive_fee`'s half is fixed and proven (`make prove`, `test_waive_fee_lost_update.py`): the decrement now computes from the stored `past_due` inside one `UPDATE`, same shape as D3. Commit `cd71243` on branch `fix/servicing-lost-update`, not yet merged — this line is due for a "merged as #N" update once it lands, per this file's own status vocabulary. `test_adjust_balance_is_still_a_separate_unlocked_read_then_write` (`services/servicing-service/tests/test_characterization_balance.py`) pins the remaining unlocked shape and runs in the blocking `atomic-apply-gate`, so fixing it later turns that gate red and sends whoever does it here. |

### D33: `applicants.ssn` is stored in plaintext, and no entry ever said so

*(Carded 2026-08-29 during an encryption-coverage sweep. Every prior SSN mention on this page is
about somewhere the SSN **goes** — the logs (D5), the redactor's separator handling (D22), the
decision inputs (D26). The column it comes from had no entry, so a reader could work through the
whole register and conclude the SSN was handled.)*

| Field | Value |
|---|---|
| **ID** | D33 |
| **Finding** | `applicants.ssn` is a plaintext `TEXT` column. The init DDL says so in a trailing comment — `ssn TEXT, -- plaintext` — which is the only place in the repository that records it. There is one shared database and one schema (ADR 0002), so all 7 services read it with the same credential; nothing scopes access to the two that need it (kyc, decision). |
| **Location** | `db/init/001_schema.sql` `applicants.ssn`. Written by `services/origination-service/app/intake.py`; read by `kyc-service` and `decision-service`. |
| **Risk** | **High.** Whole-back-book SSN exposure from a single database read — the largest data-breach blast radius on this page, larger than D13b, because every applicant has an SSN while only payers have a PAN. Exploitability is lower than D21b's was (it needs database access first, which D21a's fix has made harder), which is why the rating is High rather than Critical. It is, however, **the first thing to do among what is left**: D21b is closed, D13b is blocked on a provider decision and a card-entry surface that does not exist, D35 is waiting on a deployment, and this one is blocked on nothing but a retention answer — with a rule behind it (see Regulatory). |
| **Attribution** | **Pre-existing** (baseline schema). |
| **Regulatory** | **This is a named control requirement, not only a breach-risk judgement — and it is the half of the encryption question the client does not get to decide.** Nothing prohibits a lender from *storing* a full SSN; the opposite is true, since bureau furnishing under FCRA, TIN reporting and the CIP flow this platform implements all need it. What applies is the **GLBA Safeguards Rule, 16 CFR Part 314** (FTC, non-bank financial institutions), whose 2021 amendments added **§314.4(c)(3): encrypt all customer information "in transit over external networks and at rest"**. The at-rest half carries no qualifier, so a plaintext `applicants.ssn` sits inside it. The only exception the rule allows is compensating controls approved **in writing** by the designated Qualified Individual — an explicit, attributable act, which the platform has not performed. Depending on where the client lends, **NY DFS Part 500** (23 NYCRR 500.15, encryption at rest, CISO-approved compensating controls) may apply on top. Separately, state breach-notification statutes near-universally treat an SSN as a trigger and most provide a **safe harbor for encrypted data**, so this control is also what decides whether a future incident is reportable. Applicability is Priya's (Compliance Officer) call, not this register's — what is recorded here is that a rule is on point, so the remediation is not discretionary. |
| **Contrast worth keeping** | `decision-service` refuses to persist the SSN in `decision_events` and stores an HMAC-SHA256 under a secret pepper instead, with placeholder peppers rejected and `/health` unhealthy outside development without one (`app/decision.py::_ssn_fingerprint`, `app/config.py`). That reasoning — "an unsalted hash of a 9-digit SSN is brute-forceable" — was applied to the derived record while the source column one table over stayed in the clear. The derived-record control is correct and worth keeping; it is not a substitute. |
| **Mitigation Path** | Not a one-liner and should not be attempted as one. Options, in the order they should be evaluated: (1) **stop storing it** — establish whether anything after the KYC check and the bureau pull needs the full SSN, and keep last-4 plus the existing keyed digest if not; this is the only option that removes the risk rather than relocating it; (2) application-level encryption with a key from a secret manager, which needs a key-management answer (rotation, re-encryption, who holds the key) and touches both readers; (3) `pgcrypto` column encryption, which puts the key in reach of the same database credential that is the threat here and is the weakest of the three. Needs the client's answer on **retention duration** before any of them — but note what that answer does and does not decide. It selects *which* path: purge-to-last-4 leaves nothing to encrypt, while retaining longer makes encryption mandatory under the Regulatory row above. It does **not** decide *whether* work happens; "keep it as it is" is not one of the options. The purge path is also the only one that needs no key-management answer, which is why it is listed first. **Correction:** the purge does NOT share migration 0020's shape. 0020 dropped a column once; here the bureau pull needs the real digits while an application is still decisionable, so the column stays and the *value* is nulled per-applicant at terminal state — recurring and event-driven, not a one-shot migration. `VACUUM FULL` cannot run on every purge (`ACCESS EXCLUSIVE` on `applicants`, blocking intake and every officer read), so whether routine autovacuum reclaims the dead tuples on an acceptable schedule, or scheduled maintenance is required, is still open — resolve it before writing the real migration. |
| **Not covered** | Nulling the column does not remove the SSN from the system: pre-redaction log files (D5's open residual), database backups, WAL segments, and any replica all still hold it, and nobody has taken a retention action against those for the PAN/CVV either. The browser hop (`frontend/app/apply/page.tsx:352`) still POSTs the raw SSN in the clear — the field's password-style input is shoulder-surfing protection, not transport (D35); at-rest work does not touch it. |
| **Status** | Open. The reversible slice named in the handoff (`docs/handoffs/2026-08-31-docs-glba-encryption-framing.md`) is built: `applicants.ssn_last4` (migration `db/migrations/0023_applicants_ssn_last4.sql`, backfilled on existing rows), `recheck-kyc` and submit both send kyc-service last-4 instead of the full SSN (`app/intake.py::ssn_last4`), and the decision-service bureau pull is untouched (still reads the full column, as it must). The origination readiness rung (`db-readiness-gate`) now covers `applicants.ssn_last4`, so an unmigrated volume reports unhealthy rather than 500ing intake or recheck-kyc — that plus the blocking `ssn-purge-gate` (which pins the purge interlock and the intake last-4 hop) are the only blocking controls this entry has ever had. A purge CLI exists (`app/purge_ssn.py`) but its eligibility query is a **known-wrong placeholder** (calendar age since submission, not the application-terminal-state trigger the Mitigation Path row above requires) and is documented as such in the file's own docstring — it must not be enabled before that is reworked. That refusal is enforced in code, not only in prose: a third gate, the `_ELIGIBILITY_IS_PLACEHOLDER` module constant, makes an `--execute` run abort (exit 2) while it stands, and clearing it is a reviewed diff rather than an environment variable. It ships two operator-facing safety gates as well (`SSN_PURGE_ENABLED` env flag + explicit `--execute`; neither is set, so it has never run), and the operator procedure — including the dead-tuple question `VACUUM FULL` cannot answer for a recurring purge — is in `docs/runbooks/operations.md` and the CLI's reporting/exit-code shape, which the eventual correct query can reuse. **`applicants.ssn` itself is still plaintext and nothing has been purged.** The retention question (how long the platform must be able to re-run a bureau pull) is still open with the client; even once the purge query is corrected, it does not run until that answer lands and a human explicitly enables it. The decision record is `adr/0023-applicant-ssn-at-rest.md`; what shipped here is its Decision 1. Decision 2 — purge or encryption — is blocked on that same retention answer, and is an obligation rather than a discretionary hardening item (ADR 0023, *Sign-off status*), on a reading of the GLBA Safeguards Rule that **no counsel or Compliance Officer has yet reviewed** (ADR 0023, *Regulatory basis and review status*). |
| **Open subtask** | **Tighten the `spec-diff-gate` mapping for ADR 0023.** `scripts/spec_gate_map.txt` maps the ADR to `services/origination-service/app/intake.py` rather than to the purge mechanism the decision actually constrains, because `spec-diff-gate` is an existence check and the purge file is not on `main` yet. Remap it to `services/origination-service/app/purge_ssn.py` the day that file lands. Carded here, not only in a comment beside the map line, because no gate grades comments. |
| **Open subtask** | **Database backups and WAL segments hold the plaintext SSN, and no entry owns that.** A purge or an encryption cutover reaches the live row only; every pre-change row version survives in WAL and in any backup taken before it. D5's backup residual reads on its own terms as backups *of the pre-redaction log files*, not of the database, so it does not cover this. `db/migrations/0020_payments_drop_cvv.sql` records the identical residual for cardholder data, unowned since D13a. Carded here so closing D33 on a column purge cannot claim more coverage than the change has. Not started; no retention action exists for either dataset. Replicas fold into this subtask if one is ever deployed — none is in any committed compose file today. |

### D34: The payment request fingerprint is an unpeppered SHA-256 of the PAN

*(Carded 2026-08-29 in the same sweep. Latent today, live the moment D13b lands — recorded now so
the tokenization change inherits it instead of rediscovering it.)*

| Field | Value |
|---|---|
| **ID** | D34 |
| **Finding** | `request_fingerprint` derives its instrument component as a bare `hashlib.sha256(pan)` with no salt and no pepper, in both charge handlers (D23's duplicate). A PAN is a low-entropy input in the same sense an SSN is: the BIN is public, the last digit is a Luhn check, so a targeted issuer range leaves roughly 10⁹ candidates — the same order of magnitude as the 9-digit SSN space that `decision-service` refused to hash unkeyed. |
| **Location** | `services/payment-service/app/payments.py` and `services/servicing-service/app/payments.py`, both in `request_fingerprint` (`instrument = hashlib.sha256(pan.encode("utf-8")).hexdigest() if pan else None`). Stored in `payments.request_fingerprint`. |
| **Risk** | **Low today, Medium on D13b's merge.** Today the digest is stored in the same row as `payments.pan` in the clear, so it discloses nothing the row does not already hand over — this is not currently exploitable. That inverts when D13b removes the `pan` column: the unkeyed digest becomes the recoverable card-derived value the tokenization change left behind, in a table whose whole purpose was to stop retaining card data. The D19 entry already notes the fingerprint "hashes the PAN because no token column exists"; what it does not say is that the digest is unkeyed. |
| **Attribution** | **Ours**, introduced by the D19 idempotency fix. The pepper pattern already existed in `decision-service` at the time and was not carried across. |
| **Mitigation Path** | Fix it **inside the D13b change, not after it** — once the fingerprint covers `card_token` instead of the PAN the problem is gone by construction, and a separate pepper migration in between would need its own re-fingerprinting of live rows for no gain. If D13b slips past the next payment change that touches this function, key it with a configured pepper mirroring `DECISION_FINGERPRINT_PEPPER` (env only, no committed default, placeholder values refused, unhealthy outside development). Note the constraint: changing the fingerprint recipe changes every stored value, so a live migration must re-fingerprint or accept that in-window keys stop matching. |
| **Status** | Open; carded 2026-08-29. Not started. Tracked against **D13b**, which is where it should be fixed. |

### D35: Nothing is encrypted in transit, anywhere

*(Carded 2026-08-29 in the same sweep. Until now this page had **zero** entries mentioning TLS or
transport at all, so the absence read as an absence of the problem.)*

| Field | Value |
|---|---|
| **ID** | D35 |
| **Finding** | Four hops, none of them encrypted. (1) **Browser → gateway** is `http://localhost:8000`. (2) **Gateway → the six services** are all `http://<svc>:<port>`. (3) **Every service → Postgres**: `DATABASE_URL` carries no `sslmode`, psycopg2 (and SQLAlchemy over it) then defaults to `sslmode=prefer`, and the `postgres:16-alpine` image ships with `ssl` off and no server certificate — `prefer` never fails on the absence of TLS, so every connection negotiates down to plaintext and succeeds. (4) **Gateway → Redis** is `redis://`, not `rediss://`. Session tokens, raw continuation tokens (D21b), SSNs (D33) and PANs (D13b) all cross those hops in the clear. |
| **Location** | `.env.example` (`DATABASE_URL`, `REDIS_URL`), `services/gateway/app/config.py` (the six service URLs), `docker-compose.yml` (`NEXT_PUBLIC_GATEWAY_URL`). |
| **Risk** | **Medium as deployed today**, and the rating is entirely a function of the deployment: every hop is inside one Docker bridge network on one host, so there is no network segment to sit on that does not already imply host access. It becomes **High** the moment any hop crosses a machine — a managed Postgres, a separate Redis, a service moved to its own host, or the portal served from anywhere but the same laptop. The `COOKIE_SECURE` default is already `true` for exactly this reason, so the codebase half-anticipates the transition without the transport being there. |
| **Attribution** | **Pre-existing** (baseline compose and connection strings). |
| **Regulatory** | Same rule as D33 — **GLBA Safeguards Rule §314.4(c)(3)** — but a **weaker** attachment, and the difference is load-bearing. The rule requires encryption "in transit **over external networks** and at rest". The at-rest half (D33) has no qualifier; the in-transit half does. Every hop here is inside one Docker bridge network on one host, so today's deployment is most likely outside the in-transit requirement, and the deferral in Status below survives the rule rather than contradicting it. **That flips at the first hop that crosses a machine** — a managed Postgres, a separate Redis, a service on its own host, or the portal served from anywhere else — at which point this becomes a control failure rather than a deferred nicety. Treat the qualifier as the scheduling trigger: the work is due when the topology changes, not on a calendar. **NY DFS Part 500** (23 NYCRR 500.15) covers in-transit more broadly if the client is NY-licensed. Applicability is Priya's call. |
| **Mitigation Path** | Mostly **deployment posture, not application code**, and it should be handled as such rather than by scattering TLS settings through seven services: terminate TLS at the edge for the browser hop, and for the datastore hops require it at the connection string (`sslmode=verify-full` with a CA, `rediss://`) once a deployment exists that has certificates. The code change that is genuinely ours is small: stop letting the DSN be silent about `sslmode`, so an unencrypted database connection is a decision someone made rather than a default nobody saw. Needs the client's answer on the target deployment first — this is one of the open infrastructure questions, not a defect to fix locally. |
| **Status** | Open; carded 2026-08-29. Not started, and deliberately not scheduled: local compose is the only environment that exists, and TLS between containers on one bridge network would be ceremony. The entry exists so that the first deployment discussion starts from a written statement rather than an assumption. |
