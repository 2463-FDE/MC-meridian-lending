...
| **High** | D24: Self-decision block missed an officer's own account-linked-elsewhere submission | **Fixed** | Week 8. PR #38 review. `deny_self_decision` compared only `users.applicant_id` to `applications.applicant_id`; intake never linked the two, so an officer who self-submitted via the ordinary apply flow was not caught. Fixed with `applications.submitted_by_user_id` captured at intake (migration 0017) + a second comparison in the guard; residual is an officer who submits while NOT authenticated (out of scope, would need identity-field matching). |

---

## Week 8 governance scoping — new entry

### D24: Self-decision block only caught an officer whose account IS linked to the applicant — FIXED
...
| **Fix** | `applications.submitted_by_user_id` (migration `db/migrations/0017_applications_submitted_by_user_id.sql`, mirrored in `db/init/001_schema.sql`, with a readiness rung in `origination-service/app/config.py`). `POST /applications` now reads `X-User-Id` — the gateway forwards it for any session-bearing request, this route included — and `intake.create_application` persists it. `deny_self_decision` runs TWO independent checks: the original account-linkage comparison, plus `submitted_by_user_id == caller user id`, and blocks on either. Closes the reported scenario (officer self-submits while logged in, then self-decides) without matching on identity fields. **Residual, not this defect:** an officer who submits while NOT authenticated leaves `submitted_by_user_id` NULL and is indistinguishable from a genuine anonymous applicant — an identity-field (SSN/DOB) control would be needed to close that, out of scope here. |
| **Status** | **Fixed** on `fix/self-decision-authz` (`b775eee` tracked the gap; the fix commit closes it). Regression coverage: `test_self_submitted_application_not_linked_is_now_blocked` / `test_self_submitted_by_someone_else_is_allowed` / `test_anonymously_submitted_application_is_allowed` (`tests/test_authz.py`), route-level `test_officer_cannot_decision_their_own_self_submitted_application` (`tests/test_decision_route.py`) and `test_assistant_cannot_decision_own_self_submitted_unlinked_application` (`tests/test_assistant.py`), readiness rung tests in `tests/test_db_readiness.py`. |
