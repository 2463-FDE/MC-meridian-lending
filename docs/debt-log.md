...
| **Status** | **Partially fixed** on `fix/self-decision-authz` (`b775eee` tracked the original gap; `89bd801` closed the reported logged-in self-submit scenario). Pre-migration/genuinely-anonymous-submit rows remain open by construction; pinned (not silently) by `test_legacy_null_submitter_self_decision_is_not_blocked_by_code` (`tests/test_authz.py`) and route-level equivalents in `tests/test_decision_route.py` / `tests/test_assistant.py`. Other regression coverage: `test_self_submitted_application_not_linked_is_now_blocked` / `test_self_submitted_by_someone_else_is_allowed` / `test_anonymously_submitted_application_is_allowed` (`tests/test_authz.py`), readiness rung tests in `tests/test_db_readiness.py`. |
