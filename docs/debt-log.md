...
| **Mitigation Path** | Make the mutation a single atomic statement — `UPDATE balances SET balance = balance - :amount WHERE loan_id = :id` — committed in the same transaction as an append-only `payment_applications` row unique on `payment_id`. Specified as **D3d** in `docs/spec-payments-week5.md`; decided in **ADR 0020** (split out of ADR 0013, which named it first but does not decide it). Distinct from the idempotency key: the key collapses duplicate *intents*, but two genuinely distinct concurrent payments still race. |
| **Status** | **Open on `main` — fix built in PR #77, not yet merged.** ADR 0020 decides the fix — one statement, `payment_applications` as the record — and PR #77 implements it: migration 0019, the single-statement `apply_payment`, both callers, and a blocking `atomic-apply-gate`; `test_lost_update.py` flips from failing to passing there, proven by `make prove`. On `main` today, `balance.apply_payment` still reads the balance, subtracts in Python, and writes it back (`app/balance.py:23-31`), `ApplyPaymentIn` still accepts caller-supplied `amount` (`app/main.py:145`), migration 0019 does not exist, and `test_lost_update.py` still fails — this status line describes `main`, not PR #77, and needs updating to cite the merge commit and the gate once #77 lands. Will not fix the float column (D2), the waterfall (D14), or `adjust_balance`/`waive_fee`, which will keep the same unlocked shape — see D32. |
...
| **High** | D3: Unlocked read-modify-write on `balances` loses concurrent applies | Open on `main`; specified by ADR 0020; implementation claimed in PR [fix(servicing): apply a captured payment atomically, exactly once (D3) #77](https://github.com/2463-FDE/MC-meridian-lending/pull/77), not merged here | Week 5. Cited in code four times since the baseline, never logged and never measured. **Measured 2026-08-02:** 8 concurrent applies captured $800.00 and credited $600.00 — $200 taken, never applied. Fix is an atomic `UPDATE balances SET balance = balance - :amount` in one transaction with an append-only `payment_applications` row (ADR 0020, spec D3d). **Not fixed by the idempotency key** — distinct defect. |
...
|---|---|
| **ID** | D32 |
| **Finding** | ADR 0020 decides to convert `balance.apply_payment` to a single atomic statement, but scopes out the two sibling mutations in the same module: each reads the column, computes in Python, and writes an absolute value back, on the same shared autocommit connection with no lock and no transaction. Two concurrent calls to either one read the same opening figure and the last writer wins — the same defect as D3, one and two columns over. Carded now so the scoping decision is recorded before the D3 fix lands, not after. |
...
| **Status** | Open; documented 2026-08-23; not fixed. PR [fix(servicing): apply a captured payment atomically, exactly once (D3) #77](https://github.com/2463-FDE/MC-meridian-lending/pull/77) adds `test_the_other_mutations_are_still_a_separate_unlocked_read_then_write` (`services/servicing-service/tests/test_characterization_balance.py`) so this stays a recorded decision rather than drifting into an oversight; on `main` today, before #77 merges, that test does not exist yet. |
