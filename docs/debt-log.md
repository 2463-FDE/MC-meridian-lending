...
| **Status** | **Mitigated.** The base compose publishes no host port for postgres. Held by `scripts/tests/test_compose_datastore_ports.py` in the blocking `compose-hardening-gate`, which keys on the presence of a `ports:` key in the service block rather than on its value, so every spelling is covered. See the shared residual on D21b. |
