...
| **Closed subtask** | **`spec-diff-gate` mapping for ADR 0023 tightened.** `scripts/spec_gate_map.txt` now maps the ADR to `services/origination-service/app/purge_ssn.py`, the purge mechanism the decision actually constrains, rather than `services/origination-service/app/intake.py` — remapped once PR #142 (`25157e4`) put `purge_ssn.py` on `main`. |
