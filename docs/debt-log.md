...
| **Status** | **Mitigated.** ADR 0020 decides the fix — one statement, `payment_applications` as the record — and PR #77 (`ceda4e2`) implements it: migration 0019, the single-statement `apply_payment`, both callers, and the blocking `atomic-apply-gate`; `test_lost_update.py` flipped from failing to passing there, proven by `make prove`. Will not fix the float column (D2), the waterfall (D14), or `adjust_balance`/`waive_fee`, which keep the same unlocked shape — see D32. |
...
| **Status** | Open; documented 2026-08-23; not fixed. PR #77 (`ceda4e2`) added `test_the_other_mutations_are_still_a_separate_unlocked_read_then_write` (`services/servicing-service/tests/test_characterization_balance.py`), on `main` now, so this stays a recorded decision rather than drifting into an oversight. |
