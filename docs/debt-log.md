...
| **High** | D33: `applicants.ssn` stored in plaintext | Open (pre-existing) | Encryption sweep, 2026-08-29. `db/init/001_schema.sql` declares `ssn TEXT, -- plaintext`; one shared schema, so all 7 services read it with one credential. **No entry on this page ever covered the column** — every prior SSN row is about the logs (D5), the redactor (D22) or the decision inputs (D26), so the register read as though the SSN were handled. Largest breach blast radius here (every applicant has one; only payers have a PAN). `decision-service` already refuses to persist it unkeyed and stores a peppered HMAC in `decision_events` — correct, and not a substitute for the source column. Decide retention first: stopping storage beats encrypting it. Nothing blocking holds this. |
...
| **Medium** | D35: nothing is encrypted in transit | Open (pre-existing) | Encryption sweep, 2026-08-29. All four hops are plaintext: browser→gateway and gateway→services are `http://`, `DATABASE_URL` carries no `sslmode` so psycopg2's default `prefer` negotiates down against an image shipping `ssl` off, and Redis is `redis://` not `rediss://`. **This page carried zero TLS entries before now**, so the gap read as covered. Medium only because every hop is one Docker bridge network on one host; High the moment any hop crosses a machine. Deployment posture, not a local code fix — belongs with the open infrastructure questions. |
...
|---|---|
| **ID** | D32 |
| **Finding** | D3 made `apply_payment` one statement, but the other two mutations of the same `balances` row were deliberately left alone. `waive_fee` used to read `past_due`, subtract in Python, and write the result back — fixed (see Status). `adjust_balance` read the current value and wrote an operator-supplied figure with no predicate on what it read; also fixed (see Status), via compare-and-set rather than the delta shape the other two use, since it sets an absolute figure. |
| **Location** | `services/servicing-service/app/balance.py:163-190` (`adjust_balance`/`waive_fee`, both now atomic). Reached via `POST /accounts/{id}/adjust-balance` / `.../waive-fee` (`app/main.py`), role-gated by `authz.require_money_role`. |
| **Trigger (historical)** | An operator `adjust_balance` call landing across a concurrent `apply_payment` — the operator's blind `UPDATE balances SET balance = %s` wrote a figure computed before the payment committed, silently reversing an applied payment. Two concurrent `adjust_balance` calls on the same loan also raced, last write wins. Both closed by the compare-and-set below. |
| **Risk** | **Medium**, below D3, and now closed rather than merely bounded: `require_money_role` limited who could open the window, but the compare-and-set removes the race itself rather than relying on low volume. `adjust_balance` is still destructive by design (it overwrites the figure and records the prior value nowhere), so with no ledger (D2) and no `audit_logs` write anywhere in `servicing-service`, a *refused* attempt still can't be reconstructed after the fact — only prevented from silently landing. |
...
| **Mitigation Path** | Done for both halves. `adjust_balance` couldn't take the atomic-decrement shape as-is (it sets an absolute figure, not a delta) — it now takes `expected_balance` from the caller and does one `UPDATE ... WHERE loan_id = %s AND balance = %s RETURNING balance`; zero rows raises `BalanceChanged` (nothing written) and the route returns 409, refusing rather than guessing. Client answer on refusal UX: block and show the current balance, require a deliberate resubmit — never auto-retry a money write (`frontend/app/servicing/[loanId]/page.tsx`, `adjustBalance`/`isBalanceConflict`). Still open, separately: whether an absolute-set operation should exist at all once `payment_applications` establishes the ledger seam (D2) — that's the append-only ledger, ADR 0014, Proposed, not built. |
| **Status** | **Both halves fixed.** `waive_fee`: decrement computes from the stored `past_due` inside one `UPDATE`, same shape as D3 — commit `cd71243`, merged as PR [#138](https://github.com/2463-FDE/MC-meridian-lending/pull/138) (`530aa61`), proven by `make prove` (`test_waive_fee_lost_update.py`), held by the blocking `atomic-apply-gate`. `adjust_balance`: compare-and-set per Mitigation Path above — commit `2452f80` on branch `fix/adjust-balance-compare-and-set`, **not yet merged** (due for a "merged as #N" update once it lands, per this file's own status vocabulary), proven by `make prove` (`test_adjust_balance_refuses_when_balance_moved_underneath_caller` in `services/servicing-service/tests/test_characterization_balance.py`, plus a frontend regression in `page.test.tsx`). `test_adjust_balance_is_now_one_atomic_compare_and_set` (same file) replaces the old pinned-unlocked-shape test and runs in the blocking `atomic-apply-gate` once merged. |
...
| **Risk** | **High.** Whole-back-book SSN exposure from a single database read — the largest data-breach blast radius on this page, larger than D13b, because every applicant has an SSN while only payers have a PAN. Exploitability is lower than D21b's (it needs database access first, which D21a's fix has now made harder), which is why this is a High and not the thing to do first. |
...
| **Mitigation Path** | Not a one-liner and should not be attempted as one. Options, in the order they should be evaluated: (1) **stop storing it** — establish whether anything after the KYC check and the bureau pull needs the full SSN, and keep last-4 plus the existing keyed digest if not; this is the only option that removes the risk rather than relocating it; (2) application-level encryption with a key from a secret manager, which needs a key-management answer (rotation, re-encryption, who holds the key) and touches both readers; (3) `pgcrypto` column encryption, which puts the key in reach of the same database credential that is the threat here and is the weakest of the three. Needs the client's answer on retention before any of them. Whichever ships, the purge has the same shape as migration 0020: a rewrite (`VACUUM FULL`/`pg_repack`), not just an `UPDATE`, or the old row versions keep the values. |
| **Status** | Open; carded 2026-08-29. Not started **on this base** — no gate here asserts anything about the column. The decision record is `adr/0023-applicant-ssn-at-rest.md`; its Decision 1 (an `ssn_last4` column, KYC moved off the full value, a readiness rung, and an inert purge scaffold, all held by a blocking `ssn-purge-gate`) is implemented on the `fix/ssn-at-rest` branch and, as of 2026-08-31, is not merged to `main`. Decision 2 — purge or encryption — is blocked on the client's retention answer, and is an obligation rather than a discretionary hardening item (ADR 0023, *Sign-off status*), on a reading of the GLBA Safeguards Rule that **no counsel or Compliance Officer has yet reviewed** (ADR 0023, *Regulatory basis and review status*). |
