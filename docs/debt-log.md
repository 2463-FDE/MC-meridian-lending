...
| **Closed subtask** | **`spec-diff-gate` mapping for ADR 0023 tightened.** `scripts/spec_gate_map.txt` now maps the ADR to `services/origination-service/app/purge_ssn.py`, the purge mechanism Decision 2 actually constrains, added once PR #142 (`25157e4`) put that file on `main`. `services/origination-service/app/intake.py` keeps its line rather than being replaced: Decision 1's last-4 hop (`app/intake.py::ssn_last4`) is a built obligation of the same ADR, and `purge_ssn.py` is an inert, documented-wrong scaffold that may yet be retired — were it the only line, deleting it would leave ADR 0023 unmapped and invite an `# EXEMPT` claiming no code path while intake's hop still ships. Both lines are pinned by a regression assertion in `scripts/test_spec_diff_gate.sh`, run by the blocking `spec-diff-gate-tests` job. |
