...
| **Status** | **Open — fix specified by ADR 0020 (Week 9), not built.** `balance.apply_payment` still reads the balance, subtracts in Python, and writes it back (`app/balance.py:23-31`); `ApplyPaymentIn` still accepts caller-supplied `amount` (`app/main.py:145`); migration 0019 does not exist; `test_lost_update.py` still documents the defect and fails. ADR 0020 decides the fix — one statement, `payment_applications` as the record — and is Proposed, not Accepted, until that lands. Will not fix the float column (D2), the waterfall (D14), or `adjust_balance`/`waive_fee`, which will keep the same unlocked shape — see D32. |
