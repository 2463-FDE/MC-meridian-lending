...
- **Open, unmerged:** `fix/servicing-authz` (#32) — the only one left. It carries ADR 0014 Decision 1 and must absorb `main` before its CI reds mean anything (9 of them are week-6 characterization tests it is supposed to update).
...
- Local-only: `backup/security-pre-cleanup`, `demo/week1-4`, `wip/tracker` (the canonical `feature-status-tracker.md` lives there, not in the tracked 7-line stub on `main`).
