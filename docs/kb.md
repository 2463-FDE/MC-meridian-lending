...
**Last synced:** 2026-08-07. **Weeks 1–4 are all merged.** PRs #11 (assistant panel), #12 (week-4 disclosure), #13 (DOB) and #15 (prove-target) all merged into `main`; **only #14 (week-5 payments spec) remains open.** `main` tip is `5f32319` (2026-08-06). The local `feature/disclosure-week4` branch carries **2 post-merge commits not yet in any PR** — a tracker refresh and one disclosure fix (`e3f324c`, refuse delivery of a chain citing a non-approving decision). Plan and merge order: docs/goal-weeks1-4-closure.md (local-only, un-backticked on purpose — see below). *(Live PR metadata below — CI counts, `mergeable_state` — reflects the 2026-08-04 snapshot for the still-open #14; re-poll GitHub before trusting those numbers.)*
...
- `feature/*`, `feat/*`, `fix/*` — the user's own work on top. **Working branch: `feature/disclosure-week4`** (chronically dirty — `make prove` needs a detached worktree, never stash). Its week-4 content is already merged (#12); it now holds 2 post-merge commits awaiting a follow-up PR.
- **Open, unmerged:** `feature/payments-week5` (#14) — the only one left. (Payments is spec-only by design; see §3.)
- **Merged and done:** `feature/llm-foundation-week1` (#1), `feature/pii-redaction` (#2), `feature/llm-client-week1` (#3), `security/purge-committed-secrets` (#4), `feature/rag-eval-week2` (#5), `feature/rag-eval-impl-week2` (#6), `feature/decision-assistant-week3` (#7), `feature/langsmith-tracing` (#8), `fix/redactor-ssn-separator-blindspots` (#9), `feat/apply-form-validation` (#10), `feat/assistant-ui-panel` (#11), `feature/disclosure-week4` (#12), `fix/dob-validation` (#13), `chore/prove-target` (#15).
