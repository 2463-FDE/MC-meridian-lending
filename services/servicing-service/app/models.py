...
class Loan(Base):
    ...
    apr: Mapped[float] = mapped_column(Float)  # disclosed APR (display)
    # Contractual rate the schedule amortizes at; NULL for loans boarded before the column
    # existed, where the schedule falls back to apr (pre-change behavior).
    note_rate: Mapped[float | None] = mapped_column(Float, nullable=True)
    term_months: Mapped[int] = mapped_column(Integer)
    status: Mapped[str | None] = mapped_column(String, default="current")
    opened_at: Mapped[str | None] = mapped_column(
