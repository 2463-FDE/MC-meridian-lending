...
Stores the FULL PAN on the payments row (PCI storage debt D13b — not addressed here).
The CVV is not stored: migration 0020 dropped the column and the values (D13a). The
charge LOG is now redacted at the construction boundary: PAN/CVV/SSN are masked at the
value level before interpolation and `name` free text is not logged (mirrors
payment-service). Idempotency is enforced by a
...
def charge(
    ...
    outcome, payment_id, existing = claim_or_branch(
        idempotency_key, loan_id, pan, amount, amount_minor, method, fingerprint
    )
    if outcome != CLAIMED:
        log.info(
