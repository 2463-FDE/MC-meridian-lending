"""Per-user soft rate limit on origination-service's Bedrock-invoking routes (LLM04).

Unit-level: the counting/window logic in isolation. Route-level wiring (does each
route actually call this and surface a 429) is covered in test_assistant.py,
test_summary_route.py and test_disclosure_route.py.
"""
...
def test_calls_under_the_cap_are_allowed():
    for _ in range(rate_limit._MAX_CALLS):
        rate_limit.check_llm_rate_limit("officer-1")  # no raise


def test_the_call_past_the_cap_is_refused():
    for _ in range(rate_limit._MAX_CALLS):
        rate_limit.check_llm_rate_limit("officer-1")
...
def test_the_cap_is_per_user():
    ...
    rate_limit.check_llm_rate_limit("officer-2")  # separate bucket, no raise


def test_a_missing_user_id_shares_one_bucket_rather_than_going_unmetered():
    for _ in range(rate_limit._MAX_CALLS):
        rate_limit.check_llm_rate_limit(None)
...
def test_a_call_outside_the_window_does_not_count_against_the_cap(monkeypatch):
    ...
    rate_limit.check_llm_rate_limit("officer-1")  # window rolled, no raise
