...
def build(
    *,
    ...
    # ADR 0022 requires the report to name the data path explicitly when a
    # judge ran: retrieval-only stays the original zero-LLM-calls claim, but a
    # graded run made real Bedrock calls and must not tell the reader
    # otherwise (docs/specs/rag-week2.md D1.1 governs the retrieval-only case
    # only, not a judged one).
    if judge_backend == "none":
        provenance_line = (
            "Generated by `python -m rag_eval.run` — offline, zero LLM calls "
            "(spec D1.1). See `docs/specs/rag-week2.md`, ADR 0007, ADR 0008."
        )
    else:
        provenance_line = (
            f"Generated by `python -m rag_eval.run` — retrieval offline (spec "
            f"D1.1); conclusion/summary/prohibited verdicts graded by "
            f"{judge_backend} model `{judge_model}`, {judge_calls} LLM call(s) "
            "this run (ADR 0022)."
        )
    lines = [
