...
- **`TestClient` needs `httpx`, and two services do not pin it.** `fastapi.testclient.TestClient` raises `RuntimeError: The starlette.testclient module requires the httpx package` without it. Five services pin `httpx==0.28.1`; `servicing-service` now does too, and **`kyc-service` still does not** while `tests/test_kyc_persistence.py` uses `TestClient`. Multi-step gate jobs share one Python environment, so `kyc-enforcement-gate` and `redaction-tests` pass only because an *unrelated earlier step* in the same job installed another service's requirements. Reorder or split either job, or add a standalone gate over those files, and it fails at **collection with exit 2** — a missing package, not a broken assertion. Add the pin before adding the gate. A green local run proves nothing here: verify in a fresh venv with the gate's exact install line.
...
**Second exception: settlement reconciliation blocks.** `reconciliation-gate` runs `servicing-service` `test_reconciliation.py` + `test_reconciliation_alert.py` + `test_reconcile_cli.py` + `test_reconciliation_utc.py` + `test_runbook_reconciliation.py` outside the matrix. Reconciliation is the only thing comparing what the processor captured against what a loan was credited, and there is no prevention at capture — `payment-service` has no idempotency key, so every POST inserts a row (`services/payment-service/app/payments.py:168`). Detection is the whole control, and a regression in it is silent: the report still runs, still exits 0, still looks clean. Keep the exit codes distinct (0 clean / 1 breaks / 2 could-not-run) and keep the alert failing closed on an unset threshold — a well-formed zero from a run that could not read its inputs is the failure this gate exists to catch. **Still inside the `|| true` suppression, and able to regress on a green build: the apply-payment and balance write paths, including the red lost-update test (D3, week 9).**
