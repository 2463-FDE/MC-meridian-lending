...
- **Synchronous service coupling.** Origination blocks on downstream HTTP — a `decision-service` stall blocks the applicant-facing request. There is a timeout (`services/origination-service/app/clients.py`, one module-level 30s on every call); the bureau pull inside decision-service used to share that same 30s, so the outer budget could never expire first and a stall could not be attributed to a hop — decision-service's own bureau-pull timeout is now a strictly smaller 20s, and origination's route catches the bare-timeout case instead of letting it fall through to a generic 500. No retry or backoff anywhere in origination — correct for the billable bureau pull, unexamined for the rest. Debt D28 (partially fixed — the equal-nesting collision itself is closed; connect/read timeout split, an explicit per-class retry policy, and deadline propagation remain open); load behaviour past ~20 concurrent applications is ADR 0009 §6, deferred.
...
The `backend` matrix job runs pytest with `continue-on-error` + `|| true` — **money-math test failures there do not block the build** (known-flaky, tolerated) — but that tolerance covers the `Tests` step only. **The `Import smoke` step in the same job blocks**: it runs `python -c "import app.main"` from the service directory with no `continue-on-error`, and a bare interpreter reads no pytest ini file, so a module-level import of a repo-root package (`rag_eval`) fails there while the service's whole pytest suite stays green locally. Controls that must not regress have their own **blocking** jobs (no `continue-on-error`): `redactor-drift`, `redaction-tests`, `synthetic-credit-gate`, `decision-idempotency-gate`, `offer-guard-gate`, `adr-0010-authz-gate`, `gateway-trust-boundary-gate`, `compose-hardening-gate-tests`, `compose-hardening-gate` (no committed compose file may enable `LLM_TRACE_CONTENT`, and none may put a datastore on a host-reachable interface — the question is reachability, not `ports`, so a published port, any `network_mode`, and a network that is not provably a private bridge are all refused, graded over every file name Compose reads since `compose.yaml` takes precedence over `docker-compose.yml`; the base file may not pin the resume-cookie settings, and since #87 the demo override's own comments may not claim that boot checks an AWS credential — it does not), `kyc-enforcement-gate`, `kyc-persistence-gate`, `ssn-purge-gate` (D33: the SSN purge's placeholder interlock must keep refusing an `--execute` run, and intake must keep sending kyc-service last-4 rather than the full SSN), `payment-idempotency-gate`, `no-sad-gate`, `db-readiness-gate`, `decision-db-readiness-gate`, `migration-numbering-gate`, `disclosure-lifecycle-gate`, `atomic-apply-gate`, `assistant-telemetry-gate` (the `assistant_runs` write path, the init-DDL/migration parity block, the readiness rung, migration 0021 against a real Postgres, and the read path — the telemetry write is swallowed on failure by design, so nothing at runtime reports a volume whose migration never ran; on the read side, officer-gated `GET /assistant/metrics` is the table's only export boundary and `outcome`/`policy_band` carry no CHECK, so its vocabulary mask is the only thing keeping an unconstrained write off an officer's screen), `rag-eval-gate`, `rag-eval-import-gate`, `secret-scan` (blocks on leaked secret literals + tracked `.env`), `tila-vectors-gate`, `disclosure-narration-gate` (D37: the D1 groundedness guard on the
disclosure narration, its D2 pinned fixture set, and the D3 offline judge that grades a raw
completion against them, all run offline against `FakeAdapter` — no live model key),
`reconciliation-gate`, `agentic-loop-gate` (the officer assistant's tool-calling loop, retrieval, prompt contracts, trace content rule, injection resistance, and the five loop-swap interlocks — four in the tool closures plus PT-001's single search, whose other half refuses a policy answer from a run that never searched — previously the whole agentic surface ran under `|| true`), `doc-path-lint-tests`, `doc-path-lint`, `docs-drift-tests`, `docs-drift` (banned false-claim literals in `README.md`/`CLAUDE.md`/`docs/kb.md`), `volatile-claim-lint-tests`, `volatile-claim-lint` (a whole CLASS of decaying sentence, not a curated list), `kb-freshness-tests`, `kb-freshness` (`docs/state.md` still matches git), `spec-diff-gate` (a code area whose spec/ADR has already merged must still have one — existence, not a same-PR diff; pairings live in `scripts/spec_gate_map.txt`, and every tracked `adr/*.md` or `docs/specs/*.md` must be mapped there or carry a line-leading `# EXEMPT:` with a reason, so a new ADR is never a docs-only change), `spec-diff-gate-tests`, `retired-doc-path-lint` (no tracked file may cite a doc path retired by the docs/ reorganisation — whole tracked tree, not a doc list, since the reference that would have broken `reconciliation-gate` during that move was a path built from quoted segments inside a servicing test; no allowlist, because it asks whether a path is known-dead rather than whether it exists, and `scripts/check_retired_doc_paths.sh` plus its test are the only files excluded from their own scan), `retired-doc-path-tests`, and `frontend` build. When touching a security control (PII redaction, synthetic-credit gate, DB readiness, secrets) or the disclosure compute path, expect a dedicated blocking job — keep its regression test green. **`no-sad-gate` and `assistant-telemetry-gate` are the only jobs in the file with a database behind them** (a postgres service), because migrations 0020 and 0021 are hand-applied and "does this file run a second time" is a question only Postgres answers; each one's live vector sets `REQUIRE_LIVE_DB` so a skip fails the gate rather than exiting 0 over a check that never ran. Every other job grades SQL as text.

**The knowledge base is split by mutability, not by topic.** `docs/kb.md` holds durable prose — orientation, the read-when map, why a seam is shaped the way it is — and `docs/state.md` holds everything derivable from git: the base tip, the merged-PR ledger, the ADR file list and count, and which jobs in `ci.yml` block. `docs/state.md` is generated by `scripts/gen_state.sh` and is never hand-edited; `make kb` regenerates it and re-runs the lint. The blocking `kb-freshness` job grades it against the branch's **merge base** rather than main's tip, so a sibling PR merging does not turn every other open PR red, and the fix when it does fail is always `make kb` plus a commit. One section is deliberately not read from the base: the CI job list comes from the tree at HEAD, so a PR that adds a blocking job can show it — with the base copy, the committed page could never carry the branch's own jobs and the first regeneration after that merge would report drift on output nobody could have produced. The job runs on `pull_request` only, and that is structural: on main the merge base is the tip, so the commit that lands a regenerated page becomes the base that page would have to describe — a state no committed file can reach. The blocking `volatile-claim-lint` then refuses a decaying sentence in `README.md`/`CLAUDE.md`/`docs/kb.md` — an open-PR or not-yet-on-`main` claim, a `Last synced` stamp, a tip sha, a spelled-out count of a growing set, and (in `docs/kb.md` only) a PR number or commit id not tied to a merge on the same line. An immutable citation still passes: "merged as #77 (`ceda4e2`)" names a merge commit, and a merge commit never changes. The escape hatch is a line-trailing `<!-- VOLATILE-OK: reason -->`, and it is audited — a marker on a line that no longer violates anything fails, so the hatch cannot rot open. Why this exists: `docs/kb.md` carried ~315 hand-copied derived facts and **14 of the 25 commits that ever touched it were pure stale-fixes**; a curated list of known lies cannot close that, because whoever writes the decaying sentence is whoever would have to add it to the list.
...
**Second exception: settlement reconciliation blocks.** `reconciliation-gate` runs `servicing-service` `test_reconciliation.py` + `test_reconciliation_alert.py` + `test_reconcile_cli.py` + `test_reconciliation_utc.py` + `test_runbook_reconciliation.py` outside the matrix. Reconciliation is the only thing comparing what the processor captured against what a loan was credited. **D19 now prevents the exact-retry case at capture** — `payment-service` and `servicing-service` both claim a client-minted `idempotency_key` against a partial unique index via an insert-first `claim_or_branch()` (schema in PR #63, claim path in PR #65) before the processor is contacted, held by the blocking `payment-idempotency-gate` — but reconciliation still catches everything that isn't an exact retry under one key: a processor-side duplicate, a capture that never reached the ledger (`MISSING_IN_LEDGER`), a credit with no capture behind it (`MISSING_IN_SETTLEMENT`), an `AMOUNT_MISMATCH`, or a break that predates D19 — including a pre-#77 D3 lost update, which stays detectable historically even though the current apply-payment path can no longer produce one. Detection remains the control for those; a regression in it is silent: the report still runs, still exits 0, still looks clean. Keep the exit codes distinct (0 clean / 1 breaks / 2 could-not-run) and keep the alert failing closed on an unset threshold — a well-formed zero from a run that could not read its inputs is the failure this gate exists to catch.

**Third exception: the atomic apply blocks.** `atomic-apply-gate` runs `servicing-service` `test_lost_update.py` + `test_atomic_apply.py` + `test_payment_applications_ddl.py`, the D3 assertions in `test_characterization_balance.py`, and `payment-service` `test_apply_via_servicing.py` + `test_idempotency_vectors.py` outside the matrix (D3 / ADR 0020, week 9, merged PR #77 `ceda4e2`). `balance.apply_payment` is now one statement whose `UPDATE` computes from the stored value, recording the movement in `payment_applications` (`UNIQUE (payment_id)`) in the same statement, so concurrent applies serialize instead of overwriting each other; the lost-update test flipped from red to green and a revert turns it red again. **Both D32 halves are now fixed the same way and both regression tests have joined `atomic-apply-gate`:** `waive_fee`'s decrement computes from the stored `past_due` inside its own `UPDATE`, and `adjust_balance` is a compare-and-set — one `UPDATE ... WHERE loan_id = %s AND balance = %s` that refuses (writes nothing) when the caller's quoted balance no longer matches the stored one. See `docs/debt-log.md` D32 for the commit and merge citations.

## Docs & decisions

- ADRs in `adr/` — 0002 (single shared DB), 0003 (store card data, superseded by 0013), 0004 (service decomposition), 0005 (LLM client), 0006 (logging redaction), 0012 (Decimal/minor units, externalized rule config, FK-as-graph provenance), 0013 (payment idempotency + tokenization), 0014 (servicing money controls), 0015 (settlement reconciliation as a control), 0016 (fair-lending monitoring computes outside the platform), 0017 (self-decision anonymous-submission gap), 0018 (interim handling of a double-charged borrower), 0019 (policy retrieval on the assistant loop), 0020 (atomic payment application), 0021 (agent framework + root trace), 0022 (offline evaluator for the support test), 0023 (applicant SSN at rest). 0014, 0016 and 0017 are Proposed and specify work that is **not built**; 0015, 0019, 0020, 0021 and 0022 are Accepted and built — 0020's fix merged to `main` in PR #77 (`ceda4e2`), held by the blocking `atomic-apply-gate`, and 0021's root trace covers the officer request: `assistant.entry` opens at the route funnel (PR #85), with the loop root inside it, so a refusal raised before the loop starts is traced with an enum code. Still untraced: the gateway hop and the route's pre-funnel 403/422 refusals. ADR 0022's evaluator is `rag_eval/evaluator.py`, wired into `rag_eval/run.py` and held by the blocking `rag-eval-gate`. It grades only when asked: `RAG_JUDGE` defaults to `none`, so the keyless TF-IDF path and the gate never spend a provider call, and a run with no judge reports every model-graded axis as `not_evaluated` rather than as a pass. 0012 is Proposed but **built and merged** (PR #12, `6b395cb`) — held at Proposed on purpose, not because it's unmerged: two client answers (APR method of record, tolerance regime) can still change D1/D3, and accepting closes that door. Three others are Proposed and **partly built**: 0013, whose Decision 1 (idempotency enforced by a database constraint) shipped across #63 and #65 and whose Decision 2 is now half built — the CVV deletion shipped as migration `db/migrations/0020_payments_drop_cvv.sql` (D13a, held by the blocking `no-sad-gate`), while PAN tokenization (D13b) and Decision 3 (self-serve access) are not built, 0018, whose prevention-at-capture half is that same D19 payment-idempotency work (#63 schema, #65 capture path), held by the blocking `payment-idempotency-gate`, while borrower notification, refund submission and refund status are not built, and 0023, whose Decision 1 reduces internal exposure independently of retention — `applicants.ssn_last4`, the intake and recheck hop that sends kyc-service last-4 instead of the full SSN, and the purge CLI's placeholder interlock that refuses an `--execute` run — merged as PR #142 (`25157e4`) and held by the blocking `ssn-purge-gate` plus a `db-readiness-gate` rung, while its Decision 2 (the retention-contingent remediation) is not built and waits on a client answer on retention duration, so `applicants.ssn` is still plaintext and nothing has been purged. The ADR file list and its count are generated into `docs/state.md` by `scripts/gen_state.sh`; check the highest number there before writing a new one.
- Weeks 4–10 have merged to `main`. Week 5 and the *original* Week 6 deliverables landed as spec/ADR packages with no `services/` or `db/` change behind them — `docs/specs/payments-week5.md` and `docs/reports/servicing-money-comprehension-week6.md` are the sources of truth for what is specified but unbuilt. Week 6's follow-on authz fix is real code (`services/servicing-service/app/authz.py`), so what stays unbuilt from that week is maker-checker and the append-only ledger — the lost-update fix (D3) shipped in week 9, below. Week 4's disclosure work, week 7's reconciliation and correlation work, and week 8's governance work are real code on `main`; `docs/specs/disclosure-week4.md`, `docs/specs/observability-week7.md` and `docs/specs/fair-lending-monitoring-week8.md` are their sources of truth. Week 9 is two servicing-money fixes: D19 payment idempotency (#63 schema, #65 capture path) and the D3 atomic apply (#77, `ceda4e2`, migration `db/migrations/0019_payment_applications.sql`, held by the blocking `atomic-apply-gate`). Neither has a `docs/specs/*.md` of its own — ADR 0013 Decision 1 and `docs/specs/payments-week5.md` D1/D2 are D19's source of truth, and ADR 0020 is D3's; the week-6 comprehension doc measures the D3 defect but predates the fix and still calls it not built. Note also that spec D3(d)'s status predicate as written could never fire against the shipped D19 ordering: the built fix finalizes to `captured` first and downgrades to `captured_unapplied` when the apply refuses. Week 10 is the agentic slice: `search_policy` policy retrieval (#64, ADR 0019), one root trace over the officer assistant (#67, #68), the `policy_topic` closed-vocabulary search channel enforced before an assistant final (#71), native tool schemas and real `tool_use` blocks replacing text-encoded tool calls (#72), the loop swap to native tool calling (#76), the blocking `agentic-loop-gate` (#79), and the trace surface rendered in the frontend (#80), specified in `docs/plans/freeze-agentic-week10.md`.
- `docs/debt-log.md`, `docs/los-lss-seam.md`, `docs/runbooks/operations.md`, `docs/security-remediation-2026-07.md`.
- An "AI underwriting assistant" is planned but not built (`docs/plans/stage1-ai-assistant.md`, `docs/specs/ai-assistant-week1.md`); RAG eval harness in `rag_eval/`.
