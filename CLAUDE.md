...
**Second exception: settlement reconciliation blocks.** `reconciliation-gate` runs `servicing-service` `test_reconciliation.py` + `test_reconciliation_alert.py` + `test_reconcile_cli.py` + `test_reconciliation_utc.py` + `test_runbook_reconciliation.py` outside the matrix. Reconciliation is the only thing comparing what the processor captured against what a loan was credited, and there is still no prevention at capture — `payments` carries an `idempotency_key` under a partial unique index as of #63, but no write path claims it, so every POST inserts a row (`services/payment-service/app/payments.py:168`). Detection is the whole control, and a regression in it is silent: the report still runs, still exits 0, still looks clean. Keep the exit codes distinct (0 clean / 1 breaks / 2 could-not-run) and keep the alert failing closed on an unset threshold — a well-formed zero from a run that could not read its inputs is the failure this gate exists to catch. **Still inside the `|| true` suppression, and able to regress on a green build: the apply-payment and balance write paths, including the red lost-update test (D3, week 9).**

## Docs & decisions

- ADRs in `adr/` — 0002 (single shared DB), 0003 (store card data, superseded by 0013), 0004 (service decomposition), 0005 (LLM client), 0006 (logging redaction), 0012 (Decimal/minor units, externalized rule config, FK-as-graph provenance), 0013 (payment idempotency + tokenization), 0014 (servicing money controls), 0015 (settlement reconciliation as a control), 0016 (fair-lending monitoring computes outside the platform), 0017 (self-decision anonymous-submission gap), 0018 (interim handling of a double-charged borrower), 0019 (policy retrieval on the assistant loop). 0013, 0014, 0016 and 0017 are Proposed and specify work that is **not built**; 0015 and 0019 are Accepted and built; 0018 is Proposed and partly built (its schema rung landed with #63, the capture path did not). Nineteen ADR files exist on `main` — check the highest number before writing a new one.
